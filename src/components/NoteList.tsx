...
export default function NoteList({ site_name, section_name, page_name }) {
  ...
  const handleAddOrUpdate = async () => {
    if (editing) {
      await axios.put(`/guten/notes/${editing.id}`, { note: newNote, site_name, section_name, page_name });
      setEditing(null);
    } else {
      await axios.post("/guten/notes", { note: newNote, site_name, section_name, page_name });
    }
    setNewNote("");
      ...
      <List>
        {notes.map((note) => (
          <ListItem key={note.id} sx={{ display: "flex", flexDirection: "column", alignItems: "flex-start" }}>
            <ListItemText primary={<ReactMarkdown>{note.note}</ReactMarkdown>} />
